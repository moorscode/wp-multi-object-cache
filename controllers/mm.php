...
class WP_Object_Cache implements Object_Cache_Controller_Implementation_Interface {
	/**
	...
	public function getMulti( $keys, $groups = 'default', $server_key = '', &$cas_tokens = null, $flags = null ) {
		...
		if ( func_num_args() > 3 && ! $this->contains_no_mc_group( $groups ) ) {
			...
			if ( isset( $result ) && Memcached::RES_SUCCESS === $this->getResultCode() && is_array( $result ) ) {
	...
	public function buildKey( $key, $group = 'default' ) {
		...
		$key = preg_replace( '/\s+/', '', WP_CACHE_KEY_SALT . sprintf( '%s%s:%s', $prefix, $group, $key ) );
		if ( strlen( $key ) > 255 ) {
			$key = md5( $key );
		}
	...
	public function buildKeys( $keys, $groups = 'default' ) {
		...
		foreach ( $keys as $i => $key ) {
			...
			$group = ( 1 === $count_groups ? $groups[0] : 'default' );

			// If a group has been provided, use that group.
			if ( isset( $groups[ $i ] ) ) {
				$group = $groups[ $i ];
			}

			$derived_keys[] = $this->buildKey( $key, $group );
		}
